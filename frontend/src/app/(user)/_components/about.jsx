...
function About() {
    ...
    return (
        <footer className=' px-4 mt-8 pt-8 pb-8 md:flex space-y-6 items-center justify-between'>
            {footerContent.map((item) => {
                return <div key={item.id}
                    onClick={() => setIsOpen(item.id)}
                    className='border-b pb-4 border-b-gray-400 md:border-none'
                >
                    <h2 className='flex items-center justify-between text-secondary-800 font-bold'>{item.title}
                        <HiChevronDown className={`md:hidden ${isOpen === item.id ? "rotate-180" : ""} transition-all duration-500`} />
                    </h2>
                    <ul className={`text-sm text-secondary-600 mr-1 md:block   ${isOpen === item.id ? "" : "hidden "} `}>
                        <li>{item.desc1}</li>
                        <li>{item.desc2}</li>
                        <li>{item.desc3}</li>
                        <li>{item.desc4}</li>
            ...
            <div className='space-y-4'>
                ...
                <ul className='text-sm text-secondary-600 mr-1 space-y-4'>
                    <li><span>همراه : 09167123676</span></li>
                    ...
                    <li>
                        <a href="mailto:wwww.shahryartoolabi61@gmail.com" >shahryartoolabi@gmail.com</a>
                    </li>
                </ul>
            </div>
        </footer>
